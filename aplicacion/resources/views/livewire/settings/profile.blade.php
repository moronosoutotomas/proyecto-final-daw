...
<x-settings.layout heading="Perfil" subheading="Actualiza o teu nome e dirección de correo electrónico">
    <form wire:submit="updateProfileInformation" class="space-y-6">
        ...
        <div>
            ...
            @if (auth()->user() instanceof MustVerifyEmail &&! auth()->user()->hasVerifiedEmail())
                <div
                    class="mt-3 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md">
                    <p class="text-sm text-yellow-800 dark:text-yellow-300">
                        O teu correo electrónico non está verificado.
                        <button type="button" wire:click.prevent="resendVerificationNotification"
                            ...
                            Fai click aquí para reenvia-lo correo de verificación.
                        </button>
                    </p>

                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 text-sm font-medium text-green-600 dark:text-green-400">
                            Enviouse un novo enlace de verificación ó tu correo.
                        </p>
                    @endif
                </div>
            @endif
